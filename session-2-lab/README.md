...
1. **What happens when temperature is changed from 0.2 to 1.0?**
 The model becomes much more random and creative, so responses are less
  predictable and more varied.
2. **Why should an application not retry every API error?**
 Because some errors are permanent, such as bad input or an invalid request,
  and retrying them wastes time, money, and rate limit capacity.
3. **Why should the API key not be stored directly in the source code?**
Hard-coding the key makes it easy to leak in git history or shared code, and
  it is harder to rotate safely if it is exposed.
4. **Why does conversation history increase token usage?**
  Every new request includes the earlier messages, so the prompt gets longer
  and the model must process more tokens each turn.
5. **What is the main advantage of streaming?**
  The user starts seeing the answer immediately instead of waiting for the
  full response to finish.
6. **If 10,000 users use your application, what engineering problems might appear?**
  You may run into higher API costs, rate limits, slower response times,
  concurrency issues, database or state scaling problems, and more failure
  handling work.